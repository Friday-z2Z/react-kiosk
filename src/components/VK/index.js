 ...
 * 
 * @param {Boolean} isDisableTab    是否禁用Tab键，默认为true。
 * 
 * @param {Function} showKeyboardSetState(valueObject,reactComponent)
 ...
 * 
 * @param {Function} showKeyboard(e)
 * 显示虚拟键盘，此方式主要用于大部分模式。
 * 参数e可是3种类型
 * 1、字符串，即要联动的dom的id VirtualKeyboard.showKeyboard("inputA")
 * 2、直接是要联动的dom对象 VirtualKeyboard.showKeyboard(document.getElementById("xxx"))
 * 3、鼠标/触摸事件 <input onClick={VirtualKeyboard.showKeyboard} />
 * 
 * @param {Function} closeKeyboard()
 * 用命令来关闭键盘，一般来说用不到
 * 
 */
    ...
    render () {
        ...
        return (
            <div onContextMenu={e => e.preventDefault()} tabIndex="-1" id="VK_Main" ref={dom => this.dom = dom} style={{ outline: 'none', top: this.V.y, left: this.V.x, position: 'absolute', zIndex: '999999999', backgroundColor: '#AAA', padding: 3, width: 980, fontSize: 20, border: '1px solid #444', borderRadius: 8, userSelect: 'none' }} onMouseDown={this.F_MouseDown} onTouchStart={this.F_TouchStart} onBlur={this.F_Close}>
                <div id="VK_Title" style={{ fontSize: 14, margin: 5, textAlign: "right", cursor: 'move' }}><span id="VK_Tip" style={{ float: 'left' }}>恩普虚拟键盘</span>长按可以拖动移开或 <button style={{ borderRadius: 6, border: '1px dotted #333', cursor: 'pointer', padding: '3px 7px' }} onClick={this.F_Close}>关闭虚拟键盘</button></div>
...
export const VKB = {
    ...
    showKeyboardSetState: (valueObject, reactComponent, reactEvent) => {
        ...
        if (data.div !== null) {
            ...
            data.props.closeKeyboard = VKB.closeKeyboard
            data.props.isDisableEnter = VKB.isDisableEnter
            data.props.isDisableTab = VKB.isDisableTab
            let body = document.getElementsByTagName("body")[0]
            if (body === undefined) return
            let div = document.createElement("div")
            data.div = div
            body.appendChild(div)
            let V = <VK ref={dom => data.VK = dom} {...Vk_POSITION} values={data.props} />
